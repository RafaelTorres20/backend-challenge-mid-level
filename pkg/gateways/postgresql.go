...
// AddUserAssets implements assets.Repository.
...
func (r *PostgresRepository) GetAssetsByUserID(ctx context.Context, id string) ([]assets.Asset, error) {
	rows, err := r.db.Query("select a.name, a.price, a.currency from assets a inner join user_assets_enrollment ua on a.name = ua.asset_name where ua.user_id = $1 order by ua.position asc")
	if err != nil {
		return nil, err
	}
	...
	for rows.Next() {
		...
		err := rows.Scan(asset.Symbol, asset.Price, asset.Currency)
		if err != nil {
			return nil, err
		}
...
var queryUpsertUserAssets string = `INSERT INTO AssetUserEnrollment (user_id, asset_symbol, position)	
									VALUES ($1, $2, $3)
  									ON CONFLICT (user_id, asset_symbol) 
									DO UPDATE SET position = $3 WHERE user_id = $1 AND asset_symbol = $2`
